...
#'     and x[-n]
#'
grad_sp_idea5 <- function(xAndBeta, veccCondMeanVarObj, a, b) {
  n <- length(a)
  m <- ncol(veccCondMeanVarObj$nn) - 1
  if(m > n)
    m <- n
  x <- rep(0, n)
  beta <- rep(0, n)
  ...
  dpsi_dx <- (as.vector(t(veccCondMeanVarObj$A) %*% (beta / D + Psi / D)) -
    beta / D)[-n]
...
for(i in 1 : length(a_list_ord)){
  ...
  grad_idea_5 <- grad_sp_idea5(x0, vecc_cond_mean_var_obj, a_ord, b_ord)
...
for(i in 3 : length(a_list_ord)){
  ...
  grad_idea_5 <- grad_sp_idea5(x0, vecc_cond_mean_var_obj, a_ord, b_ord)
  grad_idea_5_epsl <- grad_sp_idea5(x0_epsl, vecc_cond_mean_var_obj, a_ord, b_ord)
