...
loglk_censor_MVN <- function(locs, indCensor, y, bCensor,
  ...
  trunc_expect <- etruncnorm(a, b / sqrt(cond_var))
...
b_censor <- 1
idx_censor <- which(y < b_censor)
loglk_censor_MVN(locs, idx_censor, y, b_censor, "matern15_isotropic",
  covparms,
  m = 50
)
