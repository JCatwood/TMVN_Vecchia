...
niter <- 30
time_df <- data.frame(matrix(NA, niter, 4 + length(m_vec) * 2))
prob_df <- data.frame(matrix(NA, niter, 4 + length(m_vec) * 2))
for (i in 1:niter) {
  cat("i = ", i, "\n")
  ## VMET ---------------------------
...
box_plt_low_dim <- function(mydf, yName = NULL,
                            yTrans = "identity") {
  mtd_names <- c(paste0("m", m_vec), paste0("m", m_vec, "_MLMC"), "MET", "TLR", "SOV", "VCDF")
