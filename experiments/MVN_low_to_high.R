...
prob1_gen <- function(n, d) {
  ...
  return(list(
    ...
    cov_name = cov_name, smoothness = smoothness
  ))
}
## Prob setups -----------------------
set.seed(123)
n_vec <- c(900, 1600, 2500) # c(900, 1600, 2500, 3600, 4900, 6400)
d <- 2
m <- 30
...
save(n_vec, time_df_Vecc, prob_df_Vecc, err_df_Vecc,
  ...
  file = "results/Vecc_low_to_high.RData"
)

# Plotting -----------------------------------
load("results/Vecc_low_to_high.RData")
library(ggplot2)
library(tidyr)
...
box_plt_low_to_high(prob_df_Vecc, yName = "MVN prob", yLim = c(0, 1e-10))
box_plt_low_to_high(time_df_Vecc, yName = "time")
